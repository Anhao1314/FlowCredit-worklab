...
本说明的基础功能表描述 H0–H3 平台；0.2 在 Executor seam 上增加两种 Reviewer、对照复核、候选处理意见和本地启动管理，M1 增加持久修复循环（REPAIR），详见 [本地平台交付说明](local-platform.md)。目标架构仍不等于已实现清单。
...
复核要求修订时：**REQUEST_REVISION → 零成本修复任务壳（同一快照）→ START_REPAIR（人显式发起）→ 新 AgentRun → 修订产物（保留谱系）→ 复核 → 候选备忘 → 等待人的判断。** 修复任务只由人显式开始，不会自动执行，也不会扩大授权。

本地平台有真实执行和持久化基础；[公开页面](https://anhao1314.github.io/FlowCredit-worklab/) 用于体验这条流程，所有模型结果都是预设模拟。当前没有持续自主工作的后台研究团队。
...
| Reviewer | 独立会话消费候选产物与引用摘录，保留意见与限制；`issues[].recordIds` 与服务端按任务快照校验，且必须来自本次实际提供的 `sourceExcerpts`，越权或未提供即整份复核被拒绝 | PASS 不是人工批准；跨执行器对照必须更换执行器，同执行器只能显式标为重复复核 |
| 修复循环（M1/M1.1） | REQUEST_REVISION 保留原任务、原产物和原复核，零成本创建绑定同一快照的修复任务壳（`snapshotId` / `snapshotOwnerId` / `contextDigest`，每次派发前重新校验）；人显式发起 START_REPAIR 后产生新 AgentRun 与新产物（携带 `supersedesArtifactId` 谱系）；修订稿走同一 Reviewer 机制 | 不自动执行、不自动切换 latest、不静默增加预算；预算不足在派发前拒绝（`WORK_BUDGET_EXHAUSTED`）；存在未完成子修复任务时放弃原任务被拒绝（`ACTIVE_CHILD_TASK_EXISTS`）；BLOCKED / 执行失败只有显式创建修复任务（存在非 PASS 复核时）与 ABANDON_TASK 两个确定性出口，不自动重试 |
| 候选备忘 | 聚合观察、引用、限制和未解决问题；修订稿的备忘记录谱系 | 不自动接纳证据，不修订正式观点，不创建研究关系 |
| 停止与恢复 | 本地保留已提交状态、修复任务与全部产物，隔离迟到结果；重启不恢复 Key | 中断且结果／费用不确定的尝试需要检查，不保证任何中断都可直接续跑 |
