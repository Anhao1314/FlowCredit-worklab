...
复核要求修订时：**REQUEST_REVISION → 修复任务（同一快照）→ 新 AgentRun → 修订产物（保留谱系）→ 复核 → 候选备忘 → 等待人的判断。** 修复任务只由人显式开始，不会自动执行，也不会扩大授权。
...
| 修复循环（M1） | REQUEST_REVISION 保留原任务、原产物和原复核，创建绑定同一快照的显式修复任务；人显式开始后产生新 AgentRun 与新产物（携带 `supersedesArtifactId` 谱系）；修订稿走同一 Reviewer 机制 | 不自动执行、不自动切换 latest、不静默增加预算；BLOCKED / 执行失败只有 CREATE_REPAIR_TASK（存在非 PASS 复核时）与 ABANDON_TASK 两个确定性出口 |
